...
impl<'a> MetricsView<'a> {
    ...
    fn format_metric_line(&self, metric: &Metric) -> Vec<String> {
        ...
        match metric {
            Metric::Gauge { name, data_points, unit } => {
            ...
            Metric::Counter { name, data_points, unit } => {
                for point in data_points {
            ...
            Metric::Histogram { name, data_points, unit } => {
                for point in data_points {
    ...
    fn get_sample_id<'b>(&self, attributes: &'b std::collections::HashMap<AttributeKey, AttributeValue>) -> Option<&'b str> {
...
impl<'a> Widget for MetricsView<'a> {
    fn render(self, area: Rect, buf: &mut Buffer) {
        ...
        let mut items = vec![ListItem::new("Metrics:").style(Style::default().add_modifier(Modifier::BOLD))];
        
        if self.metrics.is_empty() {
            items.push(ListItem::new("  No metrics received yet...").style(Style::default().fg(Color::DarkGray)));
        } else {
            // Show latest metrics
        ...
        let status_line = match self.status {
            ...
            EvaluationStatus::CollectingMetrics { received, total } => {
                match total {
